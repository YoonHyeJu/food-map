...
exports.updateStudent = async function(req, res){
  ...
  try {
    ...
    try {
      ...
      const [rows] = await indexDao.updateStudent(connection, studentIdx, studentName, major, birth, address);
...
exports.readRestaurants = async function(req, res){
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    try {
      const [rows] = await indexDao.readRestaurants(connection, category);

      return res.send({
        result: rows,
        isSuccess: true,
        code: 200, // 요청 실패시 400번대 코드
        message: "레스토랑 read 성공",
      });
    } catch (err) {
...
// 레스토랑 테이블 조회
// exports.selectRestaurant = async function (req, res){
...
//       });
//     } catch (err) {
//       logger.error(`selectRestaurant Query error\n: ${JSON.stringify(err)}`);
//       return false;
//     } finally {
//       connection.release();
//     }
//   } catch (err) {
//     logger.error(`selectRestaurant DB Connection error\n: ${JSON.stringify(err)}`);
//     return false;
//   }
// }
